...
renderer.image = (src: string, title: string, alt: string) => {
  ...
  if (alt === 'embed-youtube') {
    ...
    const fileName = srcExec.replace('/videos/', '')
    ...
    return `<div class="amp-video-wrapper">
      <amp-video controls preload="metadata" width="${width}" height="${height}" layout="responsive" poster="/videos/poster/${fileName}.${`png` ||
...
function getUrlPrefix(req: Request) {
  return req.protocol + '://' + req.headers.host
}

function enableCors(req: any, res: Response, origin: any, opt_exposeHeaders: any) {
...
function assertCors(
  ...
  const ORIGIN_REGEX = new RegExp(
    '^http://localhost:3000|' + '^https?://hrfmmymt.github.io'
  )
...
app.get('/posts/:post', (req: Request, res: Response) => {
